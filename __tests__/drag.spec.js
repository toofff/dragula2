...
describe("drag", () => {
  describe.skip("drag event gets emitted when clicking an item", () => {
    ...
    testCase(
      "works when clicking buttons by default",
      { which: 1 },
      { tag: "button", passes: true }
    );
    testCase(
      "works when clicking anchors by default",
      { which: 1 },
      { tag: "a", passes: true }
    );
    testCase("fails for right clicks", { which: 2 }, { passes: false });
    testCase(
      "fails for meta-clicks",
      { which: 1, metaKey: true },
      { passes: false }
    );
    testCase(
      "fails for ctrl-clicks",
      { which: 1, ctrlKey: true },
      { passes: false }
    );
    testCase(
      "fails when clicking containers",
      { which: 1 },
      { containerClick: true, passes: false }
    );
    testCase(
